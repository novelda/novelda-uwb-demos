...
def run_with_setup(setup_json: str):
    ...
    liveflow_fp = str(Path(__file__).resolve().parent / "Flows" / "LiveHostPlot_RadarDirectBeamforming_Python.sfl") 
    playbackflow_fp = str(Path(__file__).resolve().parent / "Flows" / "PlaybackHostPlot_RadarDirectBeamforming_Python.sfl")
    ...
    pm = ParamMaker()
    pm["RadarDirectBeamPlottingParameters"]["IsLive"] = "true" if islive else "false"
    ...
    if islive:
        ...
        if not os.path.isfile(in_fp):
            raise FileNotFoundError(f"Playback file for X7RadarDirectBeamforming not specified or not found!")
    ...
    for sec, dct in stp.items():
     ...
     # intercept azBeamAngles and sort them
    if "BeamForming" in stp:
        if "azBeamAngles" in stp["BeamForming"]:
            asstr = stp["BeamForming"]["azBeamAngles"].strip("{} ")
            az_angles = [float(a) for a in asstr.split(",")]
            az_angles = np.sort(az_angles)
            stp["BeamForming"]["azBeamAngles"] = "{" + ", ".join([f"{a:.1f}" for a in az_angles]) + "}"
    ...
    print("Running X7 RadarDirect Beamforming with preset: ", setup_json)
