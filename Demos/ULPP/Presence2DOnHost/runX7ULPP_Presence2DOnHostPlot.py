...
def run_with_setup(setup_json: str):
    ...
    liveflow_fp = str(Path(__file__).resolve().parent / "Flows" / "ULPP_Presence2DOnHost_RadarDirectLivePlot_Python.sfl") 
    playbackflow_fp = str(Path(__file__).resolve().parent / "Flows" / "ULPP_Presence2DOnHost_RadarDirectPlaybackPlot_Python.sfl")
    ...
    pm = ParamMaker()
    pm["Presence2DPlottingParameters"]["IsLive"] = "true" if islive else "false"
    ...
    if islive:
        ...
        if not os.path.isfile(in_fp):
            raise FileNotFoundError(f"Playback file for ULPPPresence2D not specified or not found!")
    ...
    print("Running X7 ULPP Presence2DHost with preset: ", setup_json)
